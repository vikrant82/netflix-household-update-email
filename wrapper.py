...
# Restart interval: 12 hours (Playwright uses on-demand browsers, no memory leak)
...
def start_target_script():
    ...
    logging.info(
        f"Attempting to start script: {TARGET_SCRIPT_NAME} using {python_executable}"
    )
    try:
        ...
        logging.info(
            f"Successfully started {TARGET_SCRIPT_NAME} with PID: {process.pid}"
        )
        return process
    except FileNotFoundError:
        logging.error(
            f"Error: Could not find the script '{TARGET_SCRIPT_NAME}'. Make sure it's in the correct directory or the path is correct."
        )
        return None
...
def stop_target_script(process: subprocess.Popen):
    ...
    pid = process.pid
    logging.info(
        f"Attempting graceful termination (SIGTERM) for PID: {pid} ({TARGET_SCRIPT_NAME})"
    )
    try:
        ...
        try:
            ...
            logging.warning(
                f"Process {pid} did not terminate gracefully after {GRACEFUL_TERMINATION_TIMEOUT_SECONDS} seconds. Forcefully killing (SIGKILL)..."
            )
            ...
            try:
                ...
                logging.error(
                    f"Process {pid} failed to be killed even with SIGKILL. Manual intervention might be required."
                )
            except Exception as e:
                logging.error(
                    f"Error waiting for process {pid} after kill: {e}", exc_info=True
                )
        except Exception as e:
            logging.error(
                f"Error waiting for process {pid} termination: {e}", exc_info=True
            )

    except Exception as e:
        logging.error(
            f"An error occurred while trying to terminate process {pid}: {e}",
            exc_info=True,
        )
        ...
        try:
            ...
            logging.error(
                f"Fallback kill attempt for PID {pid} also failed: {kill_e}",
                exc_info=True,
            )
...
def signal_handler(signum, frame):
    ...
    logging.info(
        f"Wrapper script received signal {os_signal.Signals(signum).name}. Initiating shutdown..."
    )
...
if __name__ == "__main__":
    ...
    logging.info(
        f"Restart interval: {RESTART_INTERVAL_HOURS} hours ({RESTART_INTERVAL_SECONDS} seconds)"
    )
    ...
    try:
        while True:
            ...
            if proc is None:
                logging.error(
                    "Failed to start target script. Retrying in 60 seconds..."
                )
                time.sleep(60)
            ...
            ):  # Should not happen if proc was not None, but as a safeguard
                logging.error(
                    "Target script somehow None after successful start. Retrying in 60 seconds..."
                )
                time.sleep(60)
                continue  # Retry starting
            ...
            logging.info(
                f"Target script running. Will restart around {restart_time.strftime('%Y-%m-%d %H:%M:%S')}"
            )
            ...
            try:
                ...
                logging.warning(
                    f"Target script PID {current_process.pid} exited prematurely with code {exit_code}. Restarting immediately."
                )
                ...
                logging.info(
                    f"Restart interval reached for PID {current_process.pid}. Stopping script..."
                )
                stop_target_script(current_process)
                ...
                logging.error(
                    f"An error occurred while waiting for PID {current_process.pid}: {e}",
                    exc_info=True,
                )
                ...
                logging.info(
                    "Waiting 30 seconds before attempting restart after error..."
                )
        ...
        logging.error(
            f"An unhandled error occurred in the wrapper's main loop: {e}",
            exc_info=True,
        )
    finally:
